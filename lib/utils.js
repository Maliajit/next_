...
export function resolveProductImage(product, variant = null) {
  ...
  if (!resolvedPath) {
    if (product?.heroImage) resolvedPath = product.heroImage;
    else if (product?.images && product.images.length > 0) {
        resolvedPath = Array.isArray(product.images) ? product.images[0] : (typeof product.images === 'string' ? JSON.parse(product.images)[0] : null);
    }
  }
