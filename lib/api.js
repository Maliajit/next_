...
export const toggleWishlistApi = async (userId, product) => {
  try {
    const variantId = product.variantId || product.currentVariantId || product.id;
...
export const calculateShippingApi = async (customerId, pincode) => {
  try {
    const response = await fetch(`${API_BASE_URL}/orders/calculate-shipping`, {
      method: 'POST',
      headers: { 'Content-Type': 'application/json' },
      body: JSON.stringify({ customerId, pincode }),
    });
    return await response.json();
  } catch (error) {
